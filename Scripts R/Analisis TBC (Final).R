...
SI_SE_CONTROLA_TIENE_FECHA_DE_PROXIMO_CONTROL <-  hoja_trabajo %>% filter( DNI != "") %>% filter(SE_REALIZA_CONTROLES == "Cada 15 días" | SE_REALIZA_CONTROLES == "Cada mes")  %>% filter( DNI != "") %>% count(SI_SE_CONTROLA_TIENE_FECHA_DE_PROXIMO_CONTROL)
...
control_menores <- hoja_trabajo %>% filter(DNI != "") %>% filter(INDIQUE_CUANTOS_CONVIVIENTES_MENORES_DE_19_AÑOS_TIENE != 0 & !is.na(INDIQUE_CUANTOS_CONVIVIENTES_MENORES_DE_19_AÑOS_TIENE)) %>% count(RECIBEN_TTO_PREVENTIVO_Y_CONTROLES...43)
...
# si fue, calificar calidad de atención
...
SM_esta_tto_si_1 <- hoja_trabajo %>% rename(SM_esta_en_tto = 'OFRECER DISPOSITIVO DE \nSALUD MENTAL_INDICAR SI_SE_COMPLETA_FORMS')

SM_tto_si <- SM_esta_tto_si_1 %>% count(SM_esta_en_tto,CETEC)

sheet_write(SM_tto_si,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "SM_tto_si")
...
# ahora usamos el for (comando de iteración) para bajar todas las hojas:
...
sheet_write(NECESITA_TURNO_PARA_CONTROL_MEDICO,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "NECESITA_TURNO_PARA_CONTROL_MEDICOnt")
...
sheet_write(NOMBRE_APELLIDO_DNI_TELEFONO_DE_CONVIVENTES,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "NOMBRE_APELLIDO_DNI_TELEFONO_DE_CONVIVENTES_nt")
...
sheet_write(ALGUNX_DE_SUS_FAMILIARES_CONVIVIENTES_TIENE_TUVO_DX_O_SINTOMAS_DE_TBC,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "ALGUNX_DE_SUS_FAMILIARES_CONVIVIENTES_TIENE_TUVO_DX_O_SINTOMAS_DE_TBC_nt")
...
sheet_write(SI_LA_RTA_FUE_SI_REALIZA_REALIZO_TTO,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "SI_LA_RTA_FUE_SI_REALIZA_REALIZO_TTO_nt")
...
sheet_write(INDIQUE_CUANTOS_CONVIVIENTES_MAYORES_DE_19_AÑOS_TIENE,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "INDIQUE_CUANTOS_CONVIVIENTES_MAYORES_DE_19_AÑOS_TIENE_nt")
...
sheet_write(RECIBEN_TTO_PREVENTIVO_Y_CONTROLES,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "RECIBEN_TTO_PREVENTIVO_Y_CONTROLES_nt")
...
sheet_write(TIENE_SINTOMAS_RESPIRATORIOS,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "TIENE_SINTOMAS_RESPIRATORIOS_nt")
...
sheet_write(RECIBIO_ESQUEMA_DE_VACUNA_CONTRA_COVID,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "RECIBIO_ESQUEMA_DE_VACUNA_CONTRA_COVID_nt")

sheet_write(TIENE_OOSS_O_PREPAGA,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "TIENE_OOSS_O_PREPAGA_nt")

sheet_write(SI_LA_RESPUESTA_ES_NO_TIENE_RESIDENCIA_DE_2_AÑOS_EN_PBA,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "SI_LA_RESPUESTA_ES_NO_TIENE_RESIDENCIA_DE_2_AÑOS_EN_PBA_nt")
...
SM_tto_nunca_1 <- hoja_trabajo %>% rename(SM_tto_no = 'OFRECER DISPOSITIVO DE \nSALUD MENTAL_INDICAR SI_SE_COMPLETA_FORMS')

SM_tto_no <- SM_tto_nunca_1 %>% count(SM_tto_no,CETEC)

sheet_write(SM_tto_no,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "sm_tto_nunca")
...
sheet_write(INDICAR_MOTIVOS_POR_EL_CUAL_DISCONTINUO_EL_TRATAMIENTO,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "INDICAR_MOTIVOS_POR_EL_CUAL_DISCONTINUO_EL_TRATAMIENTO_dt")
...
sheet_write(EN_QUE_FECHA_COMENZASTE_EL_TRATAMIENTO,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "EN_QUE_FECHA_COMENZASTE_EL_TRATAMIENTO_dt")
...
sheet_write(DONDE_REALIZASTE_EL_TRATAMIENTO,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "DONDE_REALIZASTE_EL_TRATAMIENTO_dt")
...
sheet_write(NOMBRE_APELLIDO_DNI_TELEFONO_DE_CONVIVENTES_f,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "NOMBRE_APELLIDO_DNI_TELEFONO_DE_CONVIVENTES_dt")
...
control_menores_disc <- hoja_trabajo %>% filter(DNI != "") %>% filter(INDIQUE_CUANTOS_CONVIVIENTES_MENORES_DE_19_AÑOS_TIENE != 0 & !is.na(INDIQUE_CUANTOS_CONVIVIENTES_MENORES_DE_19_AÑOS_TIENE)) %>% count(RECIBEN_TTO_PREVENTIVO_Y_CONTROLES...32)
...
hoja_prueba6 <- hoja_trabajo %>% rename(puerperio = 'GESTANCION / PUERPERIO') %>% rename(embarazo = '¿CURSA EMBARAZO?') %>% rename(tiempo_embarazo = '¿CUANTO TIEMPO LLEVA DE EMBARAZO? (completar en semana o trimestre)') %>% rename(controles_embarazo = '¿CUANTOS CONTROLES DE EMBARAZO REALIZO HASTA EL MOMENTO ?') %>% rename(complicaciones = '¿TUVO ALGUNA COMPLICACION DURANTE EL PUERPERIO?') %>%  rename(cual = 'SI LA RESPUESTA ES SI: INDICAR CUÁL') %>% rename (amantar = '¿ESTAS_AMAMANTANDO_?')
...
SM_tto_dt_1 <- hoja_trabajo %>% rename(SM_tto_dt = 'OFRECER DISPOSITIVO DE \nSALUD MENTAL_INDICAR SI_SE_COMPLETA_FORMS')

SM_tto_disc <- SM_tto_dt_1 %>% count(SM_tto_dt,CETEC)

sheet_write(SM_tto_disc,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "sm_tto_dt")
...
sheet_write(LE_PACIENTE_REFIERE_TENER_EL_ALTA_MEDICA,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "LE_PACIENTE_REFIERE_TENER_EL_ALTA_MEDICA_al")
...
sheet_write(doc,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "PPROFESIONAL_TRATANTE/CENTRO_DE_SALUD_DE_REFERENCIA_al")
...
# Salud Mental ya tiene el alta

SM_tto_alt <- hoja_trabajo %>% rename(SM_tto_alta = 'OFRECER DISPOSITIVO DE \nSALUD MENTAL_INDICAR SI_SE_COMPLETA_FORMS')

SM_tto_alta_1 <- SM_tto_alt %>% count(SM_tto_alta,CETEC)

sheet_write(SM_tto_alta_1,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = "sm_alta")
...
casos_RS_VI <- central %>% filter(Municipio_de_residencia == "Almirante Brown" | Municipio_de_residencia == "Avellaneda" | Municipio_de_residencia == "Berazategui" | Municipio_de_residencia == "Ezeiza" | Municipio_de_residencia == "Esteban Echeverría" | Municipio_de_residencia == "Florencio Varela" | Municipio_de_residencia == "Lanús" | Municipio_de_residencia == "Quilmes" | Municipio_de_residencia == "Lomas de Zamora") %>% mutate(CETEC = "RS VI")

casos_RS_XI <- central %>% filter(Municipio_de_residencia == "La Plata" | Municipio_de_residencia == "Presidente Perón" | Municipio_de_residencia == "San Vicente" | Municipio_de_residencia == "Berisso" | Municipio_de_residencia == "Cañuelas" | Municipio_de_residencia == "Ensenada") %>% mutate(CETEC = "RS XI")
...
# formulario derivaciones:

hoja_form <- read_sheet(ss = "1u3-Vh3atb1a-DeIsG2pQV_Iyw2atlfpZx6m2_F3TuKM",
                        sheet = "Respuestas de formulario 1",
                        col_types = "c" )

colnames(hoja_form)

# casos derivados al form

hoja_form <- hoja_form %>% rename(marca_temporal = 'Marca temporal', RS = 'RS a la que pertenece',accion = 'ACCIÓN', caso = 'CASO:',caso_tomado_por_region = 'caso tomado por región', dias_sin_tomar = 'días sin tomar') %>% filter(!is.na(marca_temporal))


casos_form <- hoja_form %>% count(marca_temporal,RS)

sheet_write(casos_form,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = 'casos_form')

accion_reaccion_form <- hoja_form %>% count(RS,accion,caso)

caso_tomado_form <- hoja_form %>% count(RS,caso_tomado_por_region,dias_sin_tomar)

sheet_write(accion_reaccion_form,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = 'accion_reaccion_form')

sheet_write(caso_tomado_form,
            ss = "1HR2RlBAjW0TBl4u1XuoOTRToGjDvY9X6Yy8x5b9-pQw",
            sheet = 'caso_tomado_form')
